...
# Do not use "virsh destroy centos68",the will cause kernel panic.
for i in {1..15}
do
	virsh start centos68
	[[ $? -eq 0 ]] && break || sleep 2
...
do 
	...
	if [[ $? -eq 1 ]];then
		...
		ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@$ip -C "curl -sL http://mirrors.rmz.gomo.com/init-instance/oz-centos68-config.sh | sh -v"
		break 
	fi
...
for i in {1..15}
do
	if [[ ! -z `virsh list --all |grep centos68 |grep -o "shut off"` ]];then
		echo -e "\033[37;32;1m qemu-img convert -p -c centos68.dsk -O qcow2 centos68-latest.qcow2 \033[0m"
		cd $OZ_HOME/images
		qemu-img convert -p -c centos68.dsk -O qcow2 centos68-latest.qcow2
		[[ $? -eq 0 ]] && break
	else
		sleep 2
		[[ $i -eq 10 ]] && echo -e "\033[37;31;1m Oh,the host seem hasn't been shut down, will execute virsh destroy... \033[0m" && virsh destroy centos68 && sleep 10 && break
	fi
done
...
cd $OZ_HOME/images/
guestmount -a  centos68-latest.qcow2 -i --rw temp/
cd temp
...
cd $OZ_HOME
umount $OZ_HOME/images/temp
rm $OZ_HOME/centos68.xml

echo -e "\033[37;33;1m Wonderfull, everything has Done. \033[0m\n"
echo -e "\033[37;34;1m Get Image:http://mirrors.rmz.gomo.com/images/centos68-latest.qcow2. \033[0m"
echo -e "\033[37;34;1m Image in:/data/www/repo/images/centos68-latest.qcow2. \033[0m"
