...
#
rpm -qa kernel
yum remove -y kernel-2.6.32-642.el6.x86_64
...
for i in {1..15}
do
	HTTP_CODE1=`curl -o /etc/yum.repos.d/epel.repo -s -w %{http_code} http://mirrors.aliyun.com/repo/epel-6.repo`
	HTTP_CODE2=`curl -o /etc/yum.repos.d/govm.repo -s -w %{http_code} http://mirrors.rmz.gomo.com/govm.repo`
	HTTP_CODE3=`curl -o /etc/yum.repos.d/CentOS-Base.repo -s -w %{http_code} http://mirrors.aliyun.com/repo/Centos-6.repo`
...
do
	yum update -y 
	res0=$?
	yum install -y acpid vim rsync cloud-utils-growpart parted lrzsz tree unzip zabbix-agent salt-minion mtr iftop tcpdump ntpdate rdate strace ntop bind-utils lsof bc telnet htop dstat redhat-lsb-core irqbalance man
	res1=$?
